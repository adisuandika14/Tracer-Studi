...
class masterkuesionerController extends Controller
{
    public function show(){
        $pertanyaan = tb_master_kuesioner::all();
    ...
    public function create(Request $request){
        ...
        $master_kuesioner = new tb_master_kuesioner();

        if($request->id_jenis ==  2){
            //$master_kuesioner->id_kuesioner = $request->id_kuesioner;
            $master_kuesioner->id_jenis = 2;
            $master_kuesioner->pertanyaan = $request->pertanyaan;
            $master_kuesioner->status = "Menunggu Konfirmasi";
            $master_kuesioner->save();
        }

        if($request->id_jenis == 1){
           // $master_kuesioner->id_kuesioner = $request->id_kuesioner;
            $master_kuesioner->id_jenis = 1;
            $master_kuesioner->pertanyaan = $request->pertanyaan;
            $master_kuesioner->status = "Menunggu Konfirmasi";
            $master_kuesioner->save();

            $master_kuesioner = tb_master_kuesioner::find(tb_master_kuesioner::max('id_master_kuesioner'));
            if($request->opsi1 != ""){
                $opsi = new tb_opsi();
                $opsi->opsi = $request->opsi1;
                $opsi->id_master_kuesioner = $master_kuesioner->id_master_kuesioner;
                $opsi->save();
            }
